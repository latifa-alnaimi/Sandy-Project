...
class maps:
    ...
    def addpoint(self, lat, lng, color = '#FF0000', title = None):
        if (lat,lng,color[1:],title) not in self.points:
            self.points.append((lat,lng,color[1:],title))

    def setInfoWindow(self, location_id, status, wind_speed = None):
        color = None
        if status == "R":    
            color = "RED"
        elif status == "Y":
            ...
            color = "GREEN"
        self.infowindows[location_id] = [color, wind_speed]
    ...
    def setInfoContent(self,title):
        content = "<b>Name:</b> %s<br><b>Status: </b>%s" %(title,self.infowindows[title][0])
        if self.infowindows[title][1] is not None:
            content = content + "<br><b>Wind Speed: </b>%s knots" %(self.infowindows[title][1])
        return content
    
    def drawpoint(self,f,lat,lon,color, title="N/A"):
        if not self.infowindows == {}:
            f.write('\t\tcontent = "%s";'%(self.setInfoContent(title)))
        ...
        f.write('\t\t\t\tinfowindow.setContent(content);\n')
        f.write('\t\t\t\tinfowindow.open(map, this);\n')
        f.write('\t\t\t}\n')
        f.write('\t\t}(content));\n')
        f.write('\n')
    
    def drawInfoWindow(self,f):
        f.write('\t\tvar content = null;\n')
